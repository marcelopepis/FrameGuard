...
import { getTweakById, getTweaksByCategory } from '../data/tweakRegistry';
...
export default function Plans() {
  ...
  const [guideOpen, setGuideOpen] = useState(() => {
    try {
      return localStorage.getItem('fg_planGuideOpen') !== 'false';
    } catch {
      return true;
    }
  });
  ...
  function toggleGuide() {
    setGuideOpen((prev) => {
      const next = !prev;
      try {
        localStorage.setItem('fg_planGuideOpen', String(next));
      } catch {}
      return next;
    });
  }
  ...
  if (view === 'edit') {
    return (
      <PlanEditor initial={editingPlan} onSave={handleSavePlan} onCancel={() => setView('list')} />
    );
  }
    ...
    <div className={styles.page}>
      ...
      {viewingPlan && (
        <PlanViewModal
          ...
          onRun={() => {
            setViewingPlan(null);
            handleExecutePlan(viewingPlan);
          }}
          onDuplicate={() => {
            setViewingPlan(null);
            handleDuplicatePlan(viewingPlan.id);
          }}
          isCompatible={isCompatible}
      ...
      {executingPlan && execState && (
        <ExecutionModal plan={executingPlan} state={execState} onClose={closeModal} />
      )}
    </div>
  );
}
      ...
      {open && (
        <div className={styles.guideContent}>
          ...
          <div className={styles.guideTip}>
            ...
            <p>
              <strong>Ordem recomendada:</strong> Manutenção Básica{' '}
              <ArrowRight size={10} strokeWidth={2.5} className={styles.guideTipArrow} /> Saúde
              Completa <ArrowRight size={10} strokeWidth={2.5} className={styles.guideTipArrow} />{' '}
              Gaming / Privacidade
            </p>
          </div>
        </div>
  ...
  return (
    <div className={styles.emptyState}>
      ...
      <p className={styles.emptyText}>
        Crie seu primeiro plano para automatizar sequências de manutenção e executar múltiplos
        tweaks com um único clique.
      </p>
      ...
      <div className={styles.cardBody} onClick={onView} role="button" tabIndex={0}>
        ...
        <div className={styles.planMeta}>
          ...
          <span className={styles.metaDate}>
            {plan.last_executed ? `Último: ${formatDate(plan.last_executed)}` : 'Nunca executado'}
          </span>
        </div>
      </div>
...
function PlanEditor({ initial, onSave, onCancel }: PlanEditorProps) {
  ...
  const [orderedIds, setOrderedIds] = useState<string[]>(() =>
    initial ? [...initial.items].sort((a, b) => a.order - b.order).map((i) => i.tweak_id) : [],
  );
  ...
  return (
    <div className={styles.page}>
      ...
      <header className={styles.header}>
        <div>
          <h1 className={styles.title}>{initial ? 'Editar Plano' : 'Novo Plano'}</h1>
      ...
      <div className={styles.editorLayout}>
        ...
        <div className={styles.editorLeft}>
          ...
          <div className={styles.tweakSection}>
            ...
            {CATEGORIES.map((cat) => {
              const tweaksInCat = getTweaksByCategory(cat.key);
              return (
                <div key={cat.key} className={styles.tweakCategory}>
                  ...
                  {tweaksInCat.map((tweak) => {
                    const vendorLabel = TWEAK_HARDWARE_MAP[tweak.id]
                      ? (
                          TWEAK_HARDWARE_MAP[tweak.id].gpu_vendor ??
                          TWEAK_HARDWARE_MAP[tweak.id].cpu_vendor ??
                          ''
                        ).toUpperCase()
                      : '';
                    return (
      ...
      <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
        ...
        <div className={styles.modalHeader}>
          <div className={styles.modalTitleGroup}>
            ...
            {plan.description && <p className={styles.modalSubtitle}>{plan.description}</p>}
          </div>
        ...
        {/* Corpo rolável: badges + descrição + lista de tarefas */}
        <div className={styles.viewBody}>
          ...
          {plan.long_description && (
            <div className={styles.viewLongDesc}>{plan.long_description}</div>
          )}

          {/* Lista de tarefas */}
          <div className={styles.viewList}>
            {sortedItems.map((item, idx) => {
              const tweak = getTweakById(item.tweak_id);
                ...
                >
                  ...
                  <div className={styles.viewItemInfo}>
                    <span className={styles.viewItemName}>
                      ...
                      {vendorLabel && <span className={styles.vendorBadge}>{vendorLabel}</span>}
                    </span>
                    {!compatible && (
        ...
        <div className={styles.viewFooter}>
          ...
          <div className={styles.viewActions}>
            ...
            <button className={styles.btnRun} onClick={onRun} disabled={enabledCount === 0}>
              <Play size={13} strokeWidth={2.5} fill="currentColor" />
              Executar
            </button>
          </div>
              ...
              <div key={item.tweak_id} className={`${styles.execItem} ${statusClass}`}>
                ...
                {isExpanded && hasDetails && (
                  <div className={styles.execItemDetails}>
                    {itemState.error && <p className={styles.execDetailError}>{itemState.error}</p>}
                    {itemState.message && (
          ...
          <div className={styles.execSummary}>
            <div className={styles.summaryStats}>
              <div className={`${styles.summaryChip} ${styles.summarySuccess}`}>
                ...
                {state.summary.completed_count} concluído
                {state.summary.completed_count !== 1 ? 's' : ''}
              </div>
...
function StatusIcon({ status, hcStatus }: StatusIconProps) {
  if (status === 'pending')
    return <Clock size={15} strokeWidth={2} className={styles.iconPending} />;
  if (status === 'running')
    return <Loader2 size={15} strokeWidth={2} className={styles.iconRunning} />;
  if (status === 'failed')
    return <XCircle size={15} strokeWidth={2} className={styles.iconFailed} />;
  if (status === 'skipped')
    return <MinusCircle size={15} strokeWidth={2} className={styles.iconSkipped} />;

  // completed — cor depende do resultado do health check
  if (hcStatus === 'warning')
    return <CheckCircle2 size={15} strokeWidth={2} className={styles.iconWarning} />;
  if (hcStatus === 'error')
    return <CheckCircle2 size={15} strokeWidth={2} className={styles.iconFailed} />;
